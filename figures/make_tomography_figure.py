...
def main():
    ...
    if not has_ci:
        # degrade gracefully: narrow synthetic band
        R_low = R_mean - 0.05
        R_high= R_mean + 0.05
    ...
    # Design t (vs 0) — back-of-envelope using absolute λ range
    lam_min = _safe_float(p.get("lambda_env_min", float(np.nanmin(lambda_env))), float(np.nanmin(lambda_env)))
    ...
    if not args.no_annot:
        ax1.text(
            ...
            rf"$(95\%\ \mathrm{{CI}}:\ {k_lo:.2f}\text{{–}}{k_hi:.2f})$" "\n"
            rf"$\gamma_{{\mathrm{{fwd}}}}={gamma_fwd:.1f}$, "
            rf"$n={{}}{n_obs}$ pts",
    ...
    # CI envelope (interpolate to a monotone x if needed)
    ...
    ax2.plot(xr, theory_y[order], ls="--", color="grey", label=theory_label)
    ...
    y_top = max(float(Rh.max()), float(theory_y.max()) if np.ndim(theory_y)>0 else float(theory_y)) + 0.05
    ax2.set_ylim(0.0, max(1.05, y_top))
    ...
    if not args.no_annot:
        # Report OLS on the plotted x
        ax2.text(
            0.02, 0.97,
            (rf"OLS: $\hat\beta={b:.3f}\pm{1.96*se_b:.3f}$, "
             rf"$\hat\alpha={a:.3f}\pm{1.96*se_a:.3f}$;  "
             rf"$R^2={R2:.2f}$" "\n" +
